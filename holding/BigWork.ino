 ...
 ***********************************/
File myFile;
int voltageR, rpmR,currentR,throttleR,tempMotR,tempDrvR,voltageL,rpmL,currentL,throttleL,tempMotL,tempDrvL, min_cell_volt_IDX, min_cell_temp, max_cell_temp, max_cell_temp_IDX;
float max_cell = 4.1;
float min_cell = 2.9;
...
void loop(void)
{
    ...
    for(int j=0;j<2;j++)
    {
      if(Can1.read(inMsgL))
      {
        ...
        if(inMsgL.id == KMC_MSG_ID_1)
        {
          ...
          rpmL = kelly_getRPM(inMsgL);
          // currentL=(inMsgL.buf[2]+256*inMsgL.buf[3])/10;
          currentL = kelly_getCurrent(inMsgL);
          // voltageL=(inMsgL.buf[4]+256*inMsgL.buf[5])/10;
          voltageL = kelly_getVoltage(inMsgL);
        }
        ...
        else if(inMsgL.id == KMC_MSG_ID_2)
        {
          ...
          throttleL = kelly_getThrottle(inMsgR);
          ...
          tempMotL = kelly_getMotorTemp(inMsgR);
          // tempDrvL= (inMsgL.buf[1]-40);        //Note this says 40 deg. offset in datasheet
          tempDrvL = kelly_getControllerTemp(inMsgR);
        }
        //BMS Baud Translation Board Output Message 1
  ...
  if(SD_flag && GPS_FIX>=5)
  {
    ...
    print_SD(rpmR,currentR,voltageR,throttleR,tempMotR,tempDrvR,rpmL,currentL,voltageL,throttleL,tempMotL,tempDrvL,digitalRead(PDOC_SIG),digitalRead(BMS_SIG), digitalRead(BSPD_SIG), digitalRead(IMD_SIG),min_cell_volt, max_cell_volt, min_cell_volt_IDX, min_cell_temp, max_cell_temp, max_cell_temp_IDX, shunt_volt, shunt_cur, low_cell_err, high_cell_err, low_cell_temp_err, high_cell_temp_err, low_shunt_volt_err, high_shunt_volt_err,Lat,Longi,GPS_Vel,GPS.hour,GPS.minute,GPS.seconds,GPS.milliseconds,GPS.day,GPS.month,GPS.year,GPS.angle,input_volt,thr_check(),digitalRead(APS_OOB__SIG),digitalRead(APS_DIS_SIG),digitalRead(LATCH_THR),digitalRead(NEG_AIR_SIG),digitalRead(COOLING_SIG),digitalRead(BRAKE_SIG),RTD_check(),tot_dist);
    SD_flag = 0;
  }
