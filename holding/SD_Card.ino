 ...
 ***********************************/
File myFile;
int VOL_R,RPM_R,CUR_R,THR_R,MTMP_R,CTMP_R,VOL_L,RPM_L,CUR_L,THR_L,MTMP_L,CTMP_L, min_cell_volt_IDX, min_cell_temp, max_cell_temp, max_cell_temp_IDX;
float max_cell = 4.1;
float min_cell = 2.9;
...
/***************************************************
 *  SETUP: Runs once to initialise everything      *
 **************************************************/
void setup()
{
  delay(100);
 // Open serial communications and wait for port to open:
  ...
  //Initialising CAN
  Can0.begin(CAN_BAUD); //set speed here.
  Can1.begin(CAN_BAUD); //set speed here.
  //Create a CAN filter that allows in only certain ID's
...
{ 
  ...
  while(Can0.available()) 
  {
    ...
    for(int j=0;j<2;j++)
    {
      if(Can0.read(inMsgR))
      {
        ...
        if(inMsgR.id==0x0CF11E05)
        {
          //Serial.println("Entered RHS 0x0CF11E05");
          RPM_R=inMsgR.buf[0]+256*inMsgR.buf[1];
          CUR_R=(inMsgR.buf[2]+256*inMsgR.buf[3])/10;
          VOL_R=(inMsgR.buf[4]+256*inMsgR.buf[5])/10;
        }
        //Right Motor Controllers Message 2
        else if(inMsgR.id==0x0CF11F05)
        {
          //Serial.println("Entered RHS 0x0CF11F05");
          THR_R = (inMsgR.buf[0]/255.0)*100; //Avoid integer division by dividing by 255.0
          MTMP_R= (inMsgR.buf[2]-40);        //Note this says 30 deg. offset in datasheet, see sensor experiment. This is cofirmed to be a 40 deg. offset
          CTMP_R= (inMsgR.buf[1]-40);        //Note this says 40 deg. offset in datasheet      
        }
  ...
  while(Can1.available()) 
  {
    ...
    for(int j=0;j<2;j++)
    {
      if(Can1.read(inMsgL))
      {
        ...
        if(inMsgL.id==0x0CF11E05)
        {
          //Serial.println("Entered LHS 0x0CF11E05");
          RPM_L=inMsgL.buf[0]+256*inMsgL.buf[1];
          CUR_L=(inMsgL.buf[2]+256*inMsgL.buf[3])/10;
          VOL_L=(inMsgL.buf[4]+256*inMsgL.buf[5])/10;
        }
        //Left Motor Controllers Message 2
        else if(inMsgL.id==0x0CF11F05)
        {
          //Serial.println("Entered LHS 0x0CF11F05");
          THR_L = (inMsgL.buf[0]/255.0)*100; //Avoid integer division by dividing by 255.0 
          MTMP_L= (inMsgL.buf[2]-40);        //Note this says 30 deg. offset in datasheet, see sensor experiment. This is cofirmed to be a 40 deg. offset
  ...
  if(SD_flag && GPS_FIX>=5)
  {
    ...
    print_SD(RPM_R,CUR_R,VOL_R,THR_R,MTMP_R,CTMP_R,RPM_L,CUR_L,VOL_L,THR_L,MTMP_L,CTMP_L,digitalRead(PDOC_SIG),digitalRead(BMS_SIG), digitalRead(BSPD_SIG), digitalRead(IMD_SIG),min_cell_volt, max_cell_volt, min_cell_volt_IDX, min_cell_temp, max_cell_temp, max_cell_temp_IDX, shunt_volt, shunt_cur, low_cell_err, high_cell_err, low_cell_temp_err, high_cell_temp_err, low_shunt_volt_err, high_shunt_volt_err,Lat,Longi,GPS_Vel,GPS.hour,GPS.minute,GPS.seconds,GPS.milliseconds,GPS.day,GPS.month,GPS.year,GPS.angle,input_volt,thr_check(),digitalRead(APS_OOB__SIG),digitalRead(APS_DIS_SIG),digitalRead(LATCH_THR),digitalRead(NEG_AIR_SIG),digitalRead(COOLING_SIG),digitalRead(BRAKE_SIG),RTD_check(),tot_dist);
    SD_flag = 0;
  }
  ...
  if(display_flag)
  {
    ...
    disp_mot_temps(MTMP_R,MTMP_L);
    // Tractive System Voltage Display
    disp_voltages(min_cell_volt,VOL_R);
    display_flag=0;
  }
}
...
void print_SD(int RPM_R_C, int CUR_R_C, int VOL_R_C, int THR_R_C, int MTMP_R_C, int CTMP_R_C, int RPM_L_C, int CUR_L_C, int VOL_L_C, int THR_L_C, int MTMP_L_C, int CTMP_L_C, bool PDOC_C, bool BMS_C, bool BSPD_C, bool IMD_C, float min_cell_volt_C, float max_cell_volt_C, int min_cell_volt_IDX_C, int min_cell_temp_C, int max_cell_temp_C, int max_cell_temp_IDX_C, float shunt_volt_C, long shunt_cur_C, bool  low_cell_err_C, bool  high_cell_err_C, bool  low_cell_temp_err_C, bool high_cell_temp_err_C, bool low_shunt_volt_err_C, bool high_shunt_volt_err_C,float Lat_C,float Longi_C,float GPS_Vel_C,int hour_C,int minute_C,int seconds_C,int milliseconds_C,int GPS_day_C,int GPS_month_C,int GPS_year_C,float GPS_angle_C,float input_volt_C,float input_throttle_C,bool APS_OOB__SIG_C,bool APS_DIS_SIG_C,bool LATCH_THR_C,bool NEG_AIR_SIG_C,bool COOLING_SIG_C,bool BRAKE_SIG_C,bool RTD_SIG_C,float running_distance_C)
{
  ...
  log_data+=",";
  log_data+=RPM_R_C*(2*pi*rad*3.6/60)*2;    //Extra *2 multiplier is because for some reason the controllers output half RPM?
  log_data+=",";
  log_data+=RPM_L_C*(2*pi*rad*3.6/60)*2;    //Extra *2 multiplier is because for some reason the controllers output half RPM?
  log_data+=",";
  ...
  log_data+=",";
  log_data+=CUR_R_C+CUR_L_C;    //Total Current Draw
  log_data+=",";
  log_data+=((CUR_R_C+CUR_L_C)*shunt_volt_C)/1000;  //Total Power Draw
